...
const AdminLoginPage = () => {
  ...
  const handleEmailSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      setLoading(true);
      const response = await axios.post(`${baseUrl}api/admin/login`, { email });
  ...
  const handleOtpSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      setShowLoading(true);
      const response = await axios.post(`${baseUrl}api/admin/verify-otp`, { otp,email });
    ...
    >
      <div className="container" style={{ maxWidth: "600px", minHeight: "400px"}}>
        <div className="card p-5">
          <h3 className="text-start mb-5">Admin, Please Login Here ...</h3>
